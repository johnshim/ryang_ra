...
# Gets thursday week before third friday in month of date
def get_expiration_date(date):
    ...
    if week_day < FRIDAY:
        second_thursday = date.replace(day = 1 + (FRIDAY - week_day) + 6)
    elif week_day > FRIDAY:
        second_thursday = date.replace(day = 1 + FRIDAY + (7 - week_day) + 6)
    elif week_day == FRIDAY:
        second_thursday = first_day.replace(day = 1 + 6)
        
    return second_thursday

# Get front month contract name by date and contract base
def get_front_month(date, contract):
    ...
    if month in MONTH_CODES.keys():
        ...
        # Roll-over occurs on first Thursday of each month
